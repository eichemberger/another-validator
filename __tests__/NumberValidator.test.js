...
describe('NumberValidator', () => {
    let validator;
    beforeEach(() => {
        validator = new NumberValidator_1.NumberValidator('test');
    });
    describe('min()', () => {
        ...
        it('should throw an error if min value is greater than max value', () => {
            validator.max(3);
            expect(() => validator.min(100)).toThrowError(messages_1.numberMessages.minGreaterThanMax);
        });
    });
    describe('max()', () => {
        ...
        it('should throw an error if max value is smaller than min value', () => {
            validator.min(100);
            expect(() => validator.max(10)).toThrowError(messages_1.numberMessages.maxSmallerThanMin);
        });
    });
    describe('validate()', () => {
    ...
    describe('getErrorMessages()', () => {
        it('should return an array of error messages', () => {
            validator.min(10).max(100);
            const expectedErrors = [messages_1.numberMessages.min];
