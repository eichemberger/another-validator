...
const buildErrorMsg_1 = require("../src/utils/buildErrorMsg");
const throwError = (v, input) => {
    const errors = v.getErrorMessages(input);
    return new ValidationError_1.ValidationError((0, buildErrorMsg_1.buildErrorMsg)(v.name), errors);
};
describe("Validator", () => {
    let validator;
    beforeEach(() => {
    ...
    it("should set minLength", () => {
        ...
        expect(validator.minLengthFlag.value).toEqual(5);
    });
    it("should have a name", () => {
        validator = new Validator_1.Validator("German");
    ...
    it("should throw error if minLength is greater than maxLength", () => {
        ...
        expect(() => validator.minLength(5)).toThrow(messages_1.messages.minLengthGreaterThanMax);
    });
    it("should throw error if minLength is less than 0", () => {
        expect(() => validator.minLength(-1)).toThrow(messages_1.messages.minLengthSmallerThanZero);
    });
    describe("isValid", () => {
    ...
    describe("assertIsValid", () => {
        it("should throw error if password length is less than minLength", () => {
            validator.minLength(6);
            expect(() => validator.assertIsValid("short")).toThrow(messages_1.messages.minLength);
        });
        describe("maxLength", () => {
            it("should throw error if password length is greater than maxLength", () => {
                validator.maxLength(10, "some message");
                expect(() => validator.validate("thisisaverylongpassword")).toThrow(throwError(validator, "thisisaverylongpassword"));
            });
            it("should throw error if max length is smaller than 1", () => {
                expect(() => validator.maxLength(0)).toThrow(messages_1.messages.maxLengthSmallerThanOne);
            });
        });
        describe("requireNumber", () => {
            it("should throw error if password does not contain a number", () => {
                validator.requireNumber();
                expect(() => validator.assertIsValid("NoNumber")).toThrow(messages_1.messages.hasNumber);
            });
        });
        describe("notNull", () => {
            it("should throw error if the input is null", () => {
                validator.notNull();
                // @ts-ignore
                expect(validator.getErrorMessages(undefined)).toEqual([messages_1.commonMessages.notNull]);
            });
        });
        describe("requireSpecialCharacter", () => {
            it("should throw error if password does not contain a special character", () => {
                ...
                expect(() => validator.assertIsValid("NoSpecialChar")).toThrow(messages_1.messages.hasSpecialCharacter);
            });
        });
        describe("noWhitespaces", () => {
            it("should throw error if password contains whitespace", () => {
                validator.noWhitespaces();
                expect(() => validator.assertIsValid("Has Whitespace")).toThrow(messages_1.messages.noWhitespaces);
            });
        });
        describe("noNumbers", () => {
            it("should throw error if password contains a number", () => {
                validator.noNumbers();
                expect(() => validator.assertIsValid("Has1Number")).toThrow(messages_1.messages.noNumbers);
            });
        });
        describe("noSpecialCharacters", () => {
            it("should throw error if password contains a special character", () => {
                validator.noSpecialCharacters();
                expect(() => validator.assertIsValid("Has@SpecialChar")).toThrow(messages_1.messages.noSpecialCharacters);
            });
        });
        describe("notBlank", () => {
        ...
        describe("onlyNumbers", () => {
            it("should throw error if password contains non-numeric characters", () => {
                ...
                expect(() => validator.assertIsValid("NotOnly1234")).toThrow(messages_1.messages.onlyNumbers);
            });
        });
        describe("noRepeatedCharacters", () => {
            it("should throw error if password contains repeated characters", () => {
                ...
                expect(() => validator.assertIsValid("Repeeeated")).toThrow(messages_1.messages.noRepeatedCharacters);
            });
        ...
        describe("onlyCharacters", () => {
            it("should throw error if password contains non-alphabetic characters", () => {
                ...
                expect(() => validator.assertIsValid("NotOnlyChars123")).toThrow(messages_1.messages.onlyCharacters);
            });
        });
        describe("isEmail", () => {
            it("should throw error if is not a valid email", () => {
                ...
                expect(() => validator.assertIsValid("NotOnlyChars123")).toThrow(message);
            });
            it("should not throw error if is a valid email", () => {
                validator.isEmail();
                expect(() => validator.assertIsValid("german@mail.com")).not.toThrow(messages_1.messages.isEmail);
            });
        });
        describe("isUrl", () => {
            it("should throw error if is not a valid url", () => {
                validator.isUrl();
                expect(() => validator.assertIsValid("NotOnlyChars123")).toThrow(messages_1.messages.isUrl);
            });
            it("should not throw error if is a valid url", () => {
                validator.isUrl();
                expect(() => validator.assertIsValid("https://google.com")).not.toThrow(messages_1.messages.isUrl);
            });
            it("should not throw error if is a valid url without http or https", () => {
                validator.isUrl();
                expect(() => validator.assertIsValid("google.com")).not.toThrow(messages_1.messages.isUrl);
            });
        });
        describe("addCustomRule", () => {
            it("should throw error if password does not meet the custom rule", () => {
                validator.addRule((password) => password.includes("test"), "custom message");
                expect(() => validator.assertIsValid("NoTestPhrase")).toThrow("custom message");
            });
        });
    });
    describe("requireUppercase and requireLowercase", () => {
        it("should throw error if password does not contain an uppercase letter", () => {
            ...
            expect(() => validator.assertIsValid("lowercase")).toThrow(messages_1.messages.hasUppercase);
        });
        it("should throw error if password does not contain a lowercase letter", () => {
            validator.requireLowercase();
            expect(() => validator.assertIsValid("UPPERCASE")).toThrow(messages_1.messages.hasLowercase);
        });
    });
    describe("fixedLength", () => {
        it("should throw error if the input does not have the right length", () => {
            ...
            expect(() => validator.validate("lowercase")).toThrow(throwError(validator, "lowercase"));
        });
